...
def get_learner_fn(
    ...
    def _update_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, Tuple]:
        ...
        def _env_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, PPOTransition]:
            ...
            raw_action, log_prob = policy.sample_and_log_prob(seed=policy_rng)
            action = jnp.tanh(raw_action)
            log_prob -= jnp.sum(
                jnp.log(0.5 * (1 - jnp.tanh(raw_action) ** 2) + 1e-6), axis=-1
            )
            ...
            transition = PPOTransition(
                done, raw_action, value, reward, log_prob, last_timestep.observation, info
            )
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                ...
                def _actor_loss_fn(
                    ...
                    log_prob -= jnp.sum(jnp.log(0.5 * (1 - jnp.tanh(traj_batch.action) ** 2) + 1e-6), axis=-1)
